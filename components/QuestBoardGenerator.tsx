...
const QuestBoardGenerator: React.FC = () => {
  ...
  // ResizeObserver untuk kalkulasi skala otomatis yang lebih stabil
  useEffect(() => {
    ...
    const updateScale = () => {
      if (previewContainerRef.current) {
         ...
         // Kita ingin 1920px (lebar asli board) muat ke container
         // Jika container 960px, maka scale 0.5.
    ...
    // Panggil sekali di awal untuk memastikan
    updateScale();

    return () => resizeObserver.disconnect();
  }, []);

  // Update window size untuk fullscreen mode
  useEffect(() => {
    const handleResize = () => {
  ...
  const renderBoardContent = (ref?: React.RefObject<HTMLDivElement | null>) => (
    <div 
      ref={ref}
      className="w-[1920px] h-[1080px] relative flex flex-col p-16 overflow-hidden bg-white shadow-2xl origin-top-left"
      style={{ 
