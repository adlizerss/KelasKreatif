...
import { Award, Crown, Star, ThumbsUp, Download, Printer, Users, FileSpreadsheet, CheckCircle, ChevronLeft, ChevronRight, Package, Palette, Check, Type, Sparkles, Share2, Copy, RotateCcw, Edit3, MoveHorizontal, MoveVertical } from 'lucide-react';
...
const CertificateGenerator: React.FC = () => {
  ...
  // Handle Resize for Certificate Preview using ResizeObserver for stability
  useEffect(() => {
    ...
    const updateScale = () => {
      if (previewContainerRef.current) {
        ...
        // Padding adjustment (approx 48px padding total from p-4 sm:p-6)
  ...
  }, [orientation]); // Re-run when orientation changes
  ...
  const processBulkPDF = async () => {
    ...
    try {
      ...
      // Calculate Image Dimensions to fit PDF centered
      let imgWidth, imgHeight;
      if (orientation === 'landscape') {
          // Landscape A4: 297 x 210
          // Source: 640 x 400 (Ratio 1.6)
          // Fit width 280 (margin ~8.5mm) -> Height 175
          imgWidth = 280;
          imgHeight = (400/640) * imgWidth;
      } else {
          // Portrait A4: 210 x 297
          // Source: 400 x 640 (Ratio 0.625)
          imgWidth = 100; // Smaller width for portrait cards usually
          imgHeight = (640/400) * imgWidth;
          // Scale up a bit for portrait card style? 100mm width is decent for a card.
          // Let's make it bigger: 140mm width
          imgWidth = 140;
          imgHeight = (640/400) * imgWidth;
      }
  ...
  return (
    <div className="animate-in fade-in duration-500 pb-10">
      <div className="text-center max-w-2xl mx-auto mb-8">
        <h2 className="text-3xl font-bold text-slate-900 dark:text-white mb-3">Generator Sertifikat</h2>
        <p className="text-slate-600 dark:text-slate-400">
          Buat kartu ucapan digital atau ID card penghargaan. Mendukung orientasi Portrait & Landscape.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden transition-colors">
            <div className="p-4 border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 flex items-center justify-between">
               <h3 className="font-semibold text-slate-800 dark:text-slate-100 flex items-center gap-2">
                 ...
                 {mode === 'single' ? '1. Pengaturan' : '1. Upload Data Massal'}
               </h3>
               {showReset && (
                   <button 
                     onClick={handleReset} 
            ...
            <div className="p-6 space-y-4">
              ...
              {mode === 'single' ? (
                ...
                <>
                  {/* Orientation Toggle */}
                  <div>
                    <label className="text-sm font-medium text-slate-700 dark:text-slate-300 block mb-2">Orientasi</label>
                    <div className="grid grid-cols-2 gap-2">
                      <button
                        onClick={() => setOrientation('portrait')}
                        className={`p-2 rounded-lg border flex items-center justify-center gap-2 transition-all ${
                          orientation === 'portrait'
                            ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-400 font-bold'
                            : 'border-slate-200 dark:border-slate-700 text-slate-500'
                        }`}
                      >
                        <MoveVertical className="w-4 h-4" /> Portrait
                      </button>
                      <button
                      ...
                      >
                        <MoveHorizontal className="w-4 h-4" /> Landscape
                      </button>
                    </div>
                  </div>
                ...
                <div className="space-y-6">
                  {/* Step 0: Orientation for Bulk */}
                  <div>
                    <label className="text-sm font-medium text-slate-700 dark:text-slate-300 block mb-2">Orientasi Output</label>
                    <div className="grid grid-cols-2 gap-2">
                      <button
                        onClick={() => setOrientation('portrait')}
                        className={`p-2 rounded-lg border flex items-center justify-center gap-2 transition-all ${
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                >
                  ...
                  PDF ({orientation === 'portrait' ? 'Portrait' : 'Landscape'})
                </button>
              </div>
            </div>
          ) : (
        ...
        <div className="lg:col-span-7 flex justify-center pb-8">
           ...
           >
              ...
              <div className="w-full flex items-center justify-between mb-4 px-2 sm:px-4 max-w-[640px]">
              ...
              <div className="relative w-full flex justify-center flex-1 items-center">
                  ...
                  >
                      <div 
                        ref={certificateRef}
                        className="relative shadow-2xl overflow-hidden flex-shrink-0 flex flex-col select-none bg-white"
                        style={{ 
                               ...
                               <div className="flex-1 p-10 flex flex-col relative">
                                  ...
                                  <div className="mt-8 flex justify-between items-end border-t border-slate-100 pt-4">
                                      ...
                                      <div className="flex flex-col items-end min-w-[200px]">
                                         ...
                                         <p className="text-sm font-bold text-slate-800 border-t border-slate-300 pt-2 w-full text-right">{data.teacherName || "Nama Guru"}</p>
